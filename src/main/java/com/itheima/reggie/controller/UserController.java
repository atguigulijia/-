...
public class UserController {
    ...
    public R<User> login(@RequestBody Map<String, String> map, HttpSession session) {
        ...
        if (StringUtils.isEmpty(phone)) return R.error("邮箱号码不能为空");
        if (StringUtils.isEmpty(code)) return R.error("验证码不能为空");
        //进行验证码比对
        if (regVerifiCode.equalsIgnoreCase(code)){
            ...
            session.removeAttribute("regVerifiCode");
            session.setAttribute("user",user.getId()); //登录后验证码失效
            return R.success(user);
        }
